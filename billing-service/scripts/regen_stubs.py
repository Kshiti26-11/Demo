"""Regenerate the vendored Orders gRPC client stubs and fix protoc's absolute import.

Usage: uv run python scripts/regen_stubs.py [PROTO_FILE] [OUT_DIR]
Defaults: contracts/upstream/orders.proto -> billing/clients/gen
"""
import re
import sys
...
if __name__ == "__main__":
    proto = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("contracts/upstream/orders.proto")
    out = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("billing/clients/gen")
    generate(proto, out)
