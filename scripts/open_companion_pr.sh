...
# Usage: scripts/open_companion_pr.sh <consumer_dir> <branch> <title> <body_file> <upstream_repo> <upstream_pr_number>
set -euo pipefail
if [ "$#" -ne 6 ]; then
  echo "usage: $0 <consumer_dir> <branch> <title> <body_file> <upstream_repo> <upstream_pr_number>" >&2
  exit 2
fi
consumer_dir=$1
branch=$2
...
url=$(gh pr create --draft --base main --head "$branch" --title "$title" --body-file "$body_abs")
