...
def emit(runs_dir: str | Path, run_id: str, step: str, agent: str, msg: str, level: str = "info",
         src: str | None = None) -> dict:
    """Append one event and return it. `src` names who produced it when that is not the step's own code
    (e.g. "bob" for lines streamed from an IBM Bob Shell session)."""
    event = {
